...
#*******************************************************;
#  2a. ACC correlation coefficients (all 137 features)  ;
#*******************************************************;
...
j_name_list            = []
#
for j in range(0,len(acc_ftrs_list)): #Extrinsic features 0 to 137
	for i in range(0,len(comp_list)): #Compounds i = 0 to 10
		print('Comp name, Extrinsic feature = ', comp_list[i], acc_ftrs_list[j])
		#print(np.argwhere(np.isnan(acc_arr[:,j]))[:,0])
		ind      = np.argwhere(~np.isnan(acc_arr[:,j]))[:,0]
		#
		pcorr_sr, pcorr_sr_pvalue = pearsonr(acc_arr[ind,j], sr_arr[ind,i])
		scorr_sr, scorr_sr_pvalue = spearmanr(acc_arr[ind,j], sr_arr[ind,i])
		...
		j_name_list.append(acc_ftrs_list[j])
...
print(df_ps)
df_ps.to_csv(path + "Plots_EPAWaters_ACC/PS_Feature_Filtering/EPAACC_Pearson_Spearsman_Coeff_Values.csv") #[1370 rows x 6 columns]
