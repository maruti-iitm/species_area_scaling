...
for j in range(0,len(wdrs_ftrs_list)): #Extrinsic features 0 to 12
	for i in range(0,len(comp_list)): #Compounds i = 0 to 10
		...
		ind      = np.argwhere(~np.isnan(wdrs_arr[:,j]))[:,0]
		#
		pcorr_sr, pcorr_sr_pvalue = pearsonr(wdrs_arr[ind,j], sr_arr[ind,i])
		scorr_sr, scorr_sr_pvalue = spearmanr(wdrs_arr[ind,j], sr_arr[ind,i])
		...
		j_name_list.append(wdrs_ftrs_list[j])
...
print(df_ps)
df_ps.to_csv(path + "Plots_WHONDRS/PS_Feature_Filtering/WHONDRS_Pearson_Spearsman_Coeff_Values.csv") #[120 rows x 6 columns]
